...
function scene:ctor(params)
	...
	res.get2("pic/bzmir/newui/login/bg.png"):addTo(self):center():fit()
...
function scene:showCommonLaunch()
	...
	self.adPageView = cc.ui.UIPageView.new({
		row = 1,
		rowSpace = 0,
		columnSpace = 0,
		column = 1,
	...
	self.sflistView = sflist_list.new(cc.size(w, display.height * 2 / 3)):add2(self):anchor(0.5, 0):pos(display.cx, 0)

	self:showRightBtns()
end

function scene:showRightBtns()
	an.newBtn(res.gettex2("pic/bzmir/newui/login/notice.png"), function()
		sound.playSound("104")
		self:showNotice()
	end, {
		pressBig = true
	}):addTo(self):pos(display.width - 90, display.height - 180)
	an.newBtn(res.gettex2("pic/bzmir/newui/login/bVLogin1.png"), function()
		sound.playSound("104")
	...
	an.newBtn(res.gettex2("pic/bzmir/newui/login/bVLogin1.png"), function()
		...
		if self.serverListData and self.serverListData then
			...
			local layer = display.newScale9Sprite(res.getframe2("pic/bzmir/newui/login/netease_bg.png"), 0, 0, cc.size(w, h)):add2(node):anchor(0.5, 0.5):pos(display.cx, display.cy)
			...
			an.newBtn(res.gettex2("pic/bzmir/newui/login/netease_btn.png"), function()
				sound.playSound("104")
...
function scene:searchServer(name)
	...
	if self.serverListData then
		...
		for i2, v2 in ipairs(self.itemDatas) do
			local p1, p2 = string.find(v2.servername, name)

			if p1 == 1 and p2 == string.len(v2.servername) then
				bHave = true
...
function scene:showNotice()
	...
	res.get2("pic/bzmir/newui/login/notice_title.png"):addTo(noticebg):pos(noticebg:getw() / 2, noticebg:geth() - 25)
	node:setTouchEnabled(true)
...
function scene:showPanel(title, inputs, btnsCfg, exitCallback)
	...
	for k2, v2 in pairs(btnsCfg) do
		...
		local btn = display.newScale9Sprite(res.getframe2("pic/bzmir/newui/login/netease_btn.png"), 0, 0, cc.size(sz.width + 24, sz.height + 5)):add2(layer):anchor(0, 0)
	...
	for k3, v3 in pairs(btns) do
		v3:pos(off2 + gap, btnY)

		off2 = off2 + gap + v3:getw()
	end
end
