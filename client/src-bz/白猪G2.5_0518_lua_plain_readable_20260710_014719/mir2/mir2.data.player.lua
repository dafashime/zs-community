local player = {
	...
	nearMemInfo = {},
	nearGroupInfo = {},
	unLimitedMoveState = def.unLimitedMoveState or 0,
	guild = {
		isAllowAlly = false,
	...
	slaves = {},
	piaoTmpDatas = {},
	setIsSplliting = function(self, inSplliting)
		self.IsSplliteItem = inSplliting
	end,
	setIsinPileUping = function(self, isinPileUping)
		self.inPileUping = isinPileUping
	end,
	setRoleID = function(self, roleid)
		self.roleid = roleid
	end,
	setIdentify = function(self, identify)
	...
	setIngot = function(self, gold)
		self.goldNum.gold = gold
	end,
	getIngot = function(self)
		return self.goldNum.gold
	end,
	setGird = function(self, gold)
		self.goldNum.gird = gold
	end,
	getGird = function(self)
		return self.goldNum.gird
	end,
	setLimitGird = function(self, gold)
		self.goldNum.limtGird = gold
	end,
	getLimitGird = function(self)
		return self.goldNum.limtGird
	end,
	setDiamond = function(self, gold)
		self.goldNum.diamond = gold
	end,
	getDiamond = function(self)
		return self.goldNum.diamond
	end,
	setSilver = function(self, gold)
		self.goldNum.silver = gold
	end,
	getSilver = function(self)
		return self.goldNum.silver
	end,
	setCapitalInfo = function(self, buf, bufLen)
...
function player:saveOldData()
	player.oldData = {}
	player.oldData.ability = {}

	local value = g_data.player.ability

	player.oldData.ability.AC = value:get("AC")
	player.oldData.ability.maxAC = value:get("maxAC")
	player.oldData.ability.MAC = value:get("MAC")
	player.oldData.ability.maxMAC = value:get("maxMAC")
	player.oldData.ability.DC = value:get("DC")
	player.oldData.ability.maxDC = value:get("maxDC")
	player.oldData.ability.MC = value:get("MC")
	player.oldData.ability.maxMC = value:get("maxMC")
	player.oldData.ability.SC = value:get("SC")
	player.oldData.ability.maxSC = value:get("maxSC")
	player.oldData.ability.maxHP = value:get("maxHP")
	player.oldData.ability.maxMP = value:get("maxMP")
end

function player:checkChange()
	attrTips_chkChange(player.oldData)
end
...
function player:getJobStr()
	return def.ccy.getJobName(self.job)
end

function player:getOtherJobStr(job)
	return def.ccy.getJobName(job)
end
...
function player:setGMUnlimitedMove(isGMUnlimitedMove)
	self.isGMUnlimitedMove = isGMUnlimitedMove
end

function player:isCrossHeroMove()
	return self.unLimitedMoveState == 2 or self.unLimitedMoveState == 3
end

function player:isCrossMonMove()
	return self.unLimitedMoveState == 1 or self.unLimitedMoveState == 3
end

function player:setUnlimitedMoveState(unLimitedMoveState)
	self.unLimitedMoveState = unLimitedMoveState
end
...
function player:getIsLeader()
	if self.guild.info then
		return ycFunction:band(ycFunction:rshift(self.guild.info:get("conferRight"), def.guild.guildPrivilege.gpOwner - 1), 1) == 1
	end
...
function player:getIsFirstLeader()
	if self.guild.info then
		return ycFunction:band(ycFunction:rshift(self.guild.info:get("conferRight"), def.guild.guildPrivilege.gpFirstOwner - 1), 1) == 1
	end
...
function player:parseLog(buf, bufLen, type)
	...
	for i = 1, bufLen do
		if string.byte(buf, i) == 0 and i ~= ibegin then
			...
			local tmpLabel2 = ycFunction:a2u(tmpLabel, i - ibegin + 1)

			strs[#strs + 1] = tmpLabel2
			ibegin = i + 1
		end
	end
...
function player:initTitle(msg, buf, bufLen)
	...
	for i2, v in ipairs(self.titleInfo) do
		if v.ActorID == titleInfo.ActorID then
			table.remove(self.titleInfo, i2)
		end
	end
...
function player:updateTitleInfo(msg, buf, bufLen)
	if msg.tag == 0 then
		...
		for i2, v2 in ipairs(self.titleInfo) do
			if v2.ActorID == self.roleid then
				for j2, t2 in ipairs(v2.data) do
					if t2:get("ID") == msg.param then
						table.remove(v2.data, j2)
					end
				end
			end
		end
