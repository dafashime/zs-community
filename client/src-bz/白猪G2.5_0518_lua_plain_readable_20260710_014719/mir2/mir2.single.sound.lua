...
function sound.getRoot()
	return def.soundRoot or sound.root
end
...
function sound.setEnable(b)
	sound.enable = b
end

function sound.preloadSound(filename)
	if not sound.enable then
		return
	end

	audio.preloadSound(sound.getRoot() .. filename .. sound.suffix)
end
...
function sound.playMusic(filename, isLoop)
	...
	if voice.isPlaying() or voice.isRecording() then
		return
	end

	audio.playMusic(sound.getRoot() .. filename .. sound.suffix, isLoop)
end
...
function sound.playSound(filename, sync)
	if not sound.enable then
		return
	end
	...
	local pos = string.find(filename, sound.suffix)
	local bHaveSuffix = pos and pos > 1 and true or false

	return audio.playSound(sound.getRoot() .. filename .. (bHaveSuffix and "" or sound.suffix), sync)
end

function sound.stopSound(handle)
	audio.stopSound(handle)
end

function sound.getFullPathWithoutAssetsPrefix(filename)
	local path = cc.FileUtils:getInstance():fullPathForFilename(sound.getRoot() .. filename .. sound.suffix)
...
function sound.setPriority(filename, priority)
...
function sound.play(type, params)
	if not sound.enable then
		return
	end
	...
	if handler then
		...
		if filename then
			local function play()
				audio.playSound(sound.getRoot() .. filename .. sound.suffix)
			end

			if delayTime then
				scheduler.performWithDelayGlobal(play, delayTime)
			else
				play()
			end
		end
	else
		audio.playSound(sound.getRoot() .. type .. "-" .. params.role.sex .. sound.suffix)
	end
end

function sound.handle_item(data)
	local stdMode = data.getVar("stdMode")

	if stdMode == 0 then
		if data.getVar("shape") ~= 3 then
			sound.playSound(sound.s_click_drug)
		else
			sound.playSound(sound.s_itmclick)
		end
	elseif stdMode == 31 then
		if checkIn(data.getVar("aniCount"), 1, 3) then
			sound.playSound(sound.s_click_drug)
		else
			sound.playSound(sound.s_itmclick)
		end
	...
	elseif checkExist(stdMode, 24, 26) then
		...
		if string.find(name, "手镯") or string.find(name, "手套") then
...
function sound.handle_appr(soundid)
	return soundid
end

function sound.handle_sitdown(soundid)
	return soundid
end

function sound.handle_scream(soundid)
	return soundid
end

function sound.handle_born(soundid)
	return soundid
end

function sound.handle_mon(params)
...
function sound.handle_mon_weapon(params)
	local hiter = params.act.hiter
	local role = params.map:findRole(hiter)

	if role then
		if not checkExist(role:getRace(), 0, 1, 150) then
			return
		end
		...
		if def.weapon_struck_config then
			filename = cuswpsound(weapon)
		elseif checkExist(weapon, 6, 20) then
...
function sound.handle_hit(params)
	local role = params.role
	...
	elseif effect and effect.magicId then
		...
		if def.weapon_hit_config then
			filename = cuswpsound(weapon, true)
...
function sound.handle_footStep(params)
	...
	local data = map.file:gettile(math.floor(role.x / 2) * 2, math.floor(role.y / 2) * 2)
	...
	local ret = (function()
		...
		local idx2 = data.objFileIdx * 10000 + idx - 1
		...
		elseif checkExist(idx2, 3230, 3231, 3246, 3277) then
		...
		if checkIn(idx2, {
			825,
			1349
		}) and math.floor((idx2 - 825) / 25) % 2 == 0 then
			footstepsound = sound.s_walk_stone_l
		end
		...
		if checkIn(idx2, {
			1375,
			1799
		}) and math.floor((idx2 - 1375) / 25) % 2 == 0 then
			footstepsound = sound.s_walk_cave_l
		end
		...
		local idx3 = ycFunction:band(data.mididx, 32767) - 1
		...
		local idx4 = ycFunction:band(data.objidx, 32767) - 1
...
function sound.handle_hited(params)
	local role = params.role
	local delay = params.delay
	local filename

	if role.sex == 0 then
		filename = sound.s_man_struck
	elseif role.sex == 1 then
		filename = sound.s_wom_struck
	end

	return filename, delay and delay / 2 or nil
end

function sound.handle_skillSpell(params)
	local role = params.role
...
function sound.handle_skillPlay(params)
	local magicId = tonumber(params.magicId)
...
function sound.monSounds(appr)
	local ret = 200 + appr * 10
