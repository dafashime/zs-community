...
function cls:selectServer(ip, port)
	self.ip = ip
	self.port = port
	self.serverUrl = string.format("http://%s:%s/", ip, port)
end
...
function cls:send_(op, params, reqfunc)
	self:_getXhr().responseType = cc.XMLHTTPREQUEST_RESPONSE_STRING
...
function cls:register(acc, psw, safecode, agent)
...
function cls:onEvt(code, desc, res, ex)
	if self.evtListener then
		self.evtListener:onCetSvrEvt(code, desc, res, ex)
	end
end
