function main(skillIndex)
	...
	if testWalkTo or testAll then
		...
		function EVT.waitEvt()
			HELPER:say("正在等待玩家")
		end

		function EVT.failtEvt()
			HELPER:say("目标不可达")
			HELPER:followPlayer()
		end
	...
	if testRole then
		...
		for k = 0, 7 do
			...
			actor = createRole("施法 人物_" .. k, "hero", 24, 19, 1, 0, pos(5 + cfg[1] * 2, 8 + cfg[2] * 2), nil)
			actor.dir = k
		...
		for _ = 1, 3 do
			for y = -15, 15 do
				for x = 0, 40 do
					...
					elseif move then
						d.role.node:runs({
							action.rep(action.seq(d:actPlayAct(ACTS.WALK, pos(-1, -1)), d:actDelay(2)), 50)
						})
					end
