...
function btnEx:loadMain()
	...
	local allwidth2 = allwidth * scale
	local space2 = space * scale
	...
	for i, v in ipairs(btns) do
		local btn = an.newBtn(res.gettex2("pic/console/iconbg11.png"), function()
			...
			pressBig = true,
			sprite = res.gettex2("pic/console/panel-icons/" .. v .. bzmir.ext)
		}):pos(display.cx - allwidth2 / 2 + (i - 0.5) * space2, h / 2):scale(scale):add2(self.content)
