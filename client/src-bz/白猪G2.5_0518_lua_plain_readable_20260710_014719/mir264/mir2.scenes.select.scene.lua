	...
	if device.platform == "android" then
		...
		self:addNodeEventListener(cc.KEYPAD_EVENT, function(event)
			if event.key == "back" then
				an.newMsgbox("确定要「退出游戏」吗？", function(idx)
					if idx == 1 then
						os.exit(1)
					end
				end, {
...
function scene:onEnterTransitionFinish()
	...
	res.get2("pic/bzmir/newui/login/select.png"):addTo(layer):pos(layer:getw() / 2, layer:geth() / 2)
	...
	for i = 1, 5 do
		...
		if i == 1 then
			local value
			local btn = an.newBtn(res.gettex2("pic/bzmir/newui/login/tab1.png"), function()
				clickBtn(i)
			end, {
				pressImage = res.gettex2("pic/bzmir/newui/login/tab2.png")
			}):pos(x, y):addto(layer)

			if self.smallExitState then
		...
		elseif i == 5 then
			an.newBtn(res.gettex2("pic/bzmir/newui/login/tab9.png"), function()
				self.returnBtn = true

				clickBtn(i)
			end, {
				pressImage = res.gettex2("pic/bzmir/newui/login/tab10.png")
			}):pos(x, y):addto(layer)
		else
			an.newBtn(res.gettex2("pic/bzmir/newui/login/tab" .. 2 * i - 1 .. ".png"), function()
				clickBtn(i)
			end, {
				pressImage = res.gettex2("pic/bzmir/newui/login/tab" .. 2 * i .. ".png")
			}):pos(x, y):addto(layer)
		end
	end
	...
	for i2 = 1, 2 do
		an.newBtn(res.getuitex(1, 66), function()
			clickSelect((self.pageIdx - 1) * 2 + i2)
		end, {
			pressShow = true
		}):pos(172 + (i2 - 1) * 551, 130):addto(layer)
	end
	...
	for i3 = 1, 2 do
		self.pageBtn[i3] = an.newBtn(res.getuitex(3, 401 + (i3 - 1) * 2), function()
			clickPage(i3)
		end, {
			pressImage = res.getuitex(3, 402 + (i3 - 1) * 2)
		}):addTo(layer):pos(layer:getw() / 2 + 3, 400 - (i3 - 1) * 60)

		self.pageBtn[i3]:setVisible(#g_data.select.roles > 2)
	end
...
function scene:showCreate()
	...
	if def.openMultiJob then
		if def.maxJob > 5 then
			def.maxJob = 5
		end

		number = number - 2
	elseif g_data.login.localLastSer.smallzone then
		number = number - string.len(g_data.login.localLastSer.smallzone)
	end

	local editBox1 = an.newInput(145, 300, 150, 30, number, {
		donotMove = true,
		donotClip = true
	}):addTo(edit)
	local model
	...
	local sexIndex
	local btns = {}
	local value = def.maxJob or 3

	local function clickBtn(idx)
		if workIndex == 8 + (idx - value) + 1 or workIndex == idx or idx - value == sexIndex then
			return
		end
		...
		for i = 1, value do
			if i <= 3 then
				...
				btns[i]:setIsSelect(workIndex == 8 + (i - value) + 1)
			end
		end

		for index = value + 1, value + 2 do
			btns[index]:setIsSelect(sexIndex == index - value)
		end
	...
	for i2 = 1, value + 2 do
		...
		if value > 3 then
			if i2 > 3 and i2 <= value then
				i3 = 500 + (i2 - 3)
			elseif value < i2 then
				i3 = i2 - (value - 3)
			end
		end

		btns[i2] = an.newBtn(res.getuitex(1, 74 + i3 - 1), function()
			sound.playSound("104")
			clickBtn(i2)
		end, {
			pressBig = true,
			select = {
				res.getuitex(1, 55 + i3 - 1),
				manual = true
			}
		}):pos(69 + (i2 <= value and (i2 - 1) * 45 or (i2 - value) * 45), i2 <= value and 242 or 170):addto(edit)
	end
	...
	local function random()
		if not firstName or not boyName or not girlName then
			an.newMsgbox("随机生成名字的配置文件格式有误，无法生成。请手动输入", nil, {
				center = true
			})
	...
	an.newBtn(res.getuitex(1, 62), function()
		...
		if editBox1:getText() ~= "" then
			local newRoleName = editBox1:getText()

			if def.openMultiJob then
				newRoleName = "N" .. workIndex - 1 .. newRoleName
			elseif getsmallzone then
				newRoleName = getsmallzone(newRoleName)
			end
			...
			if workIndex > 3 then
				local text = def.jobMaps[tostring(workIndex - 1)]

				if text and text.use then
					if text.use == "MC" then
						count = 1
					elseif text.use == "SC" then
						count = 2
					end
				end
			end
			...
			net.send({
				...
				hair = 1,
				name = newRoleName,
				job = workIndex > 3 and count or workIndex - 1,
				sex = sexIndex - 1
			}))
			hideCreate()
		else
...
function scene:createInfo(idx, info)
	...
	local jobbyName = func.getJobbyName(info.name)
	local value = info.work
	local value2 = info.work - 1

	if jobbyName then
		value2 = jobbyName.job
		value = jobbyName.job + 1
	end

	info.model = role.new(value, info.sex):setState(g_data.select.selectIndex == idx and "normal" or "stone")
	...
	an.newLabel(getJobStr(value2), 16, 1):pos(x, y - 62):addto(info.layer)
end
...
function scene:receiveDelChrs(msg, buf, bufLen)
	...
	for i = 1, count do
		...
		self.del_roles[#self.del_roles + 1] = {
			name = temp:get("name"),
			job = func.getJobbyName(temp:get("name")) and func.getJobbyName(temp:get("name")).job or temp:get("job"),
			hair = temp:get("hair"),
...
function scene:ShowDelChrList()
	...
	for k, v in ipairs(self.del_roles) do
		...
		labels[#labels + 1] = an.newLabel(v.name, 14, 1):addTo(cell):pos(45, cell:geth() / 2):anchor(0.5, 0.5)
		labels[#labels + 1] = an.newLabel(v.level, 14, 1):addTo(cell):pos(113, cell:geth() / 2):anchor(0.5, 0.5)
		labels[#labels + 1] = an.newLabel(getJobStr(v.job), 14, 1):addTo(cell):pos(155, cell:geth() / 2):anchor(0.5, 0.5)
		labels[#labels + 1] = an.newLabel(getSexStr(v.sex), 14, 1):addTo(cell):pos(200, cell:geth() / 2):anchor(0.5, 0.5)
...
function scene:ekeyErrorMsg(key)
	...
	elseif key == 110 then
		...
		ret = key == 3 and "因为短时间内多次输入错误密码或其它原因，暂时被禁止登录" or "未知密宝验证错误[" .. key .. "]"
	end
...
function scene:queueUp(pos, cnt, sec)
	...
	if not layer.queueUpTip then
		...
		res.get2("pic/bzmir/newui/login/queue.png"):addTo(bg):pos(bg:getw() / 2, bg:geth() - 6):anchor(0.5, 1)
