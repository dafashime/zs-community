...
function voice:loadChannelList()
	...
	local function load()
		...
		local curSelect, selectSpr
		local cnt, lineh = 0, 38
	...
	function createVoice()
		...
		msgbox = an.newMsgbox("", function(idx)
			...
			net.send({
				...
				needPw = msgbox.pwInput:getString() == "" and 0 or 1,
				pw = msgbox.pwInput:getString(),
				memberMax = mem
			}))
		end, {
			hasCancel = true,
...
function voice:loadMemberList(isPreview, channelType, roomData, members)
	...
	if not isPreview then
		channelType, roomData, members = g_data.voice.channelType, g_data.voice.roomData, g_data.voice.members
	end
	...
	local membersLists = {}
	local memberSelectSpr, memberSelectData
	...
	function self.content.addMember(v)
		...
		local lists, scroll, lineh

		if isAdmin then
			lists, scroll, lineh = adminLists, adminScroll, 32
		else
			lists, scroll, lineh = membersLists, membersScroll, 38
		end
