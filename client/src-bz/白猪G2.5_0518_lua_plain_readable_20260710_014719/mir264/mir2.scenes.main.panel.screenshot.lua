...
function screenshot:showTools()
	...
	local function add(x, y)
		if x - size / 2 < 0 or y - size / 2 < 0 or x + size / 2 > self.canvas:getw() or y + size / 2 > self.canvas:geth() then
			return
		end
	...
	touchNode:addNodeEventListener(cc.NODE_TOUCH_EVENT, function(event)
		...
		local x, y = event.x - rect1.x - rect2.x, event.y - rect1.y - rect2.y
		...
		elseif event.name == "moved" then
			...
			local nextx, nexty = lastpos.x, lastpos.y
