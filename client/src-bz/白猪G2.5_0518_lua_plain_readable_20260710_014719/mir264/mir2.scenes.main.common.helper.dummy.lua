...
function dummy:ctor(name, race, dress, weapon, sex, hair, offset, realPos, time)
	...
	if not realPos then
		if offset.off2 then
			model.x, model.y = offset.x, offset.y
		else
...
function dummy:walkTo_(x, y, mapid, tip, guideDis, maxDis, waitEvt, arriveEvt, faildEvt)
	...
	if dis > (guideDis or dummy.GUIDE_DISTANCE_LIMIT) then
		if dis > (maxDis or dummy.FOLLOW_DISTANCE_LIMIT) then
