...
function mixingDrug:ctor()
	...
	for _, v2 in ipairs(self.tabs.btns) do
		v2.text = an.newLabel("", 15, 2):addTo(v2):pos(v2:getw() / 2, 2):anchor(0.5, 0)
	end
...
function mixingDrug:showMixingLv(lv, curExp, maxExp)
	local layer = self.layer
	local w = 15
	local h = 5 + an.newLabel("熟练度:" .. curExp .. "/" .. maxExp, 18, 1, {
		color = cc.c3b(196, 186, 137)
	}):addTo(layer):pos(w, 5):anchor(0, 0):geth() + 5
...
function mixingDrug:showMixingBegin(data, id)
	...
	local label = an.newLabel(data.time * cnt .. "分钟", 18, 1):addTo(itembg):pos(itembg:getw() / 2, itembg:geth() / 2 + 9):anchor(0.5, 0)
	local label2 = an.newLabel("需要金币" .. data.price * cnt, 18, 1, {
	...
	function modify(num)
		...
		input:setText(cnt .. "")
		label:setText(data.time * cnt .. "分钟")
		label2:setText("需要金币" .. data.price * cnt)
	end
...
function mixingDrug:showMixingDuring(data, id)
	...
	local label = an.newLabel("剩余" .. math.ceil((data.time or 1) / 60) .. "分钟", 18, 1, {
		color = cc.c3b(225, 98, 13)
	...
	self.handler = scheduler.scheduleGlobal(function()
		...
		if data.time > 0 then
			label:setString("剩余" .. math.ceil((data.time or 1) / 60) .. "分钟")
		else
			self:stopHandler()
			self:refresh()
		end
	...
	res.get2("pic/panels/mixingDrug/effect_1.png"):addTo(layer):pos(layer:centerPos()):anchor(0.5, 0.5):playAnimationForever(display.newAnimation(frames, 0.125))
end
