...
function fusion:showReplaceTips(scenePos)
	...
	if p.x + w > rect.width then
		p.x = rect.width - w
	end

	if p.y > rect.height then
		p.y = rect.height
	end
...
function fusion:showSimpleTips(targetList, scenePos)
	...
	for i, v in ipairs(targetList) do
		...
		local AC, maxAC, MAC, maxMAC = data:getVar("AC"), data:getVar("maxAC"), data:getVar("MAC"), data:getVar("maxMAC")
		local ACN, maxACN, MACN, maxMACN = data:getVar("AC"), data:getVar("maxAC"), data:getVar("MAC"), data:getVar("maxMAC")
