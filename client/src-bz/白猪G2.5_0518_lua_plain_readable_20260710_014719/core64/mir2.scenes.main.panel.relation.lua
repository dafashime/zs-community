...
function chatListView:createChatItem(itemData)
	...
	if itemData.time - self.lastChatTime > 300 then
	...
	local posY2 = posY + nameItem:geth() + self.itemInterval
	local chatData = common.decodeMsg(itemData.text)
	local chatItem2 = common.createChatLabel({
		strWidth = 320,
		channel = "私聊",
		data = chatData,
		user = itemData.name,
	...
	for k, v in pairs(chatData) do
		local voice = chatItem2:findVoiceBubbleForMsgID(v.msgID)
	...
	if #chatItem2.lines == 1 then
		local l = chatItem2.lines[1]
		local max = 0

		for k2, v2 in ipairs(l:getChildren()) do
	...
	chatItem2:pos(itemData.isOther and 20 or self.viewWidth - itemWidth - 20, -posY2 - 12)

	n.chatLabel = chatItem2
	...
	local pnl = display.newScale9Sprite(res.getframe2(string.format("pic/scale/chatBubble%d.png", itemData.isOther and 1 or 2)), offsetX, -8, cc.size(itemWidth + 30, chatItem2:geth() + 24)):anchor(itemData.isOther and 0 or 1, 0):add2(chatItem2, -1)

	pnl:setCapInsets(cc.rect(itemData.isOther and 59 or 15, 23, 5, 5))

	local posY3 = posY2 + self.itemInterval + pnl:geth()
...
function chatListView:initChatList(defaultItemCnt)
	...
	local defaultItemCnt2 = defaultItemCnt or 10

	for k = 1, defaultItemCnt2 do
		local item = iter()
...
function relation:showContentFriend(parent)
	...
	for i2, v2 in ipairs(dataList) do
		local cell = display.newScale9Sprite(res.getframe2(i2 % 2 == 0 and "pic/scale/scale18.png" or "pic/scale/scale19.png"), 0, 0, cc.size(188, h)):anchor(0, 0):pos(0, friendView:getScrollSize().height - i2 * h):add2(friendView)
		local label = v2:get("name")
		...
		local label2 = v2:get("level")

		an.newLabel(label2, 18, 1, {
			color = relation.uiColor.cellNor
	...
	self.keyboard = an.newInput(10, base:geth() / 2, 320, 40, 50, {
		...
		keyboardEx = {
			get = function()
				...
				return keyboardEx2.create(self.keyboard)
			end,
			remove = function()
...
function relation:showContentNear(parent, list)
	...
	for i2, v2 in ipairs(dataList) do
		...
		if type(v2) ~= "string" then
			...
			level = v2:get("level")
			job = v2:get("job")
			job = def.ccy.getJobName(job)
			sex = v2:get("sex") == 0 and "男" or "女"
			guild = self:fixStrLen(v2:get("guildName") or "", 11)
		end

		local cell = display.newScale9Sprite(res.getframe2(i2 % 2 == 0 and "pic/scale/scale18.png" or "pic/scale/scale19.png"), 0, 0, cc.size(596, h)):anchor(0, 0):pos(0, friendView:getScrollSize().height - i2 * h):add2(friendView)
		local label = name
		...
		an.newLabel(label2, itemFontSize, 1, {
			color = relation.uiColor.cellNor
		}):add2(cell):anchor(0.5, 0.5):pos(220, h * 0.5)

		local label3 = job

		an.newLabel(label3, itemFontSize, 1, {
			color = relation.uiColor.cellNor
		}):add2(cell):anchor(0.5, 0.5):pos(300, h * 0.5)

		local label4 = sex

		an.newLabel(label4, itemFontSize, 1, {
			color = relation.uiColor.cellNor
		}):add2(cell):anchor(0.5, 0.5):pos(380, h * 0.5)

		local label5 = guild

		an.newLabel(label5, itemFontSize, 1, {
			color = relation.uiColor.cellNor
...
function relation:showContentAttention(parent)
	...
	for i2, v2 in ipairs(dataList) do
		...
		local label3 = def.ccy.getJobName(v2:get("job"))

		an.newLabel(label3, itemFontSize, 1, {
			color = relation.uiColor.cellNor
		}):add2(cell):anchor(0.5, 0.5):pos(380, h * 0.5)

		local colorIndex = v2:get("color")
...
function relation.onAddFriendOk(buf, recog)
	if buf then
		main_scene.ui:tip(string.format(" %s同意成为您的好友", net.str(buf)))
	end
end

function relation.onAddFriendFail(buf, recog)
	local value = buf and net.str(buf) or "对方"
	local tip = {
		[-2] = "不能添加自己为好友",
		[-4] = "好友人数达到上限",
		[-1] = string.format("%s不在线或输入名错误", value),
		[-3] = string.format("%s已是好友", value),
		[-5] = string.format("添加%s失败", value)
	}
...
function relation.joinGroup(name)
	...
	main_scene.ui:tip(string.format("申请加入 %s 的队伍", name))
end
