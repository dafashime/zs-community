...
function deal:addItem(who, data)
	...
	if who == "self" then
		...
		local idx2 = #self.targetItems + 1

		self.targetItems[idx2] = item.new(data, self, {
			idx = idx2
		}):addto(self.targetLayer):pos(self:idx2pos(idx2))
	end
end
...
function deal:putItem(item2, x, y)
	if item2.formPanel.__cname == "bag" then
		...
		if g_data.client:checkLastTime("deal", 4) then
			...
			item2.formPanel:delItem(data:get("makeIndex"))
