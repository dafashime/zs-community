...
function autoRat:updateModifyProperty()
	...
	if not g_data.setting.item.filt then
		local items = {}
	end

	local items2 = def.items
...
function autoRat:checkCounterAttack()
	local number = g_data.player.ability:get("atkRoleid")

	if number then
		local number2 = main_scene.ground.map:findRole(tonumber(number))

		if number2 and not number2.die and getDis(main_scene.ground.player, number2) < 8 then
			return number2
		else
			g_data.player.ability:set("atkRoleid", nil)
		end
	end
...
function autoRat:getNearestTarget(noItem)
	...
	if g_data.setting.autoRat.autoCounterAttack then
		local target = self:checkCounterAttack()

		if not target or target.die or g_data.hero and g_data.hero.roleid == target.roleid then
			-- block empty
		else
			...
			if self.target.roleid ~= target.roleid then
				self.target = target
			end

			self.target.isCounterAttack = true

			return target
		end
	end

	local value = main_scene.ground.map.mons or {}
	...
	local items = main_scene.ground.map.items or {}
		...
		for _2, item in pairs(items) do
			if item.owner == player.roleid and not self:getTempData(item, "cannotPick") and getDis(player, item) <= 10 then
				...
				if wei < maxWei and g_data.bag:getFreeCount() > 0 or itemName == "金币" then
					if item.state and item.state > 0 and pickGoodAttItem then
	...
	table.sort(targets, function(a, b)
		...
		if self:isMonster(a) then
			...
			if a.last and a.last.state and def.role.stateHas(a.last.state, "stStone") then
				disA = disA + 3
			end
		elseif a.state and a.state > 0 and pickGoodAttItem then
		...
		if self:isMonster(b) then
			...
			if b.last and b.last.state and def.role.stateHas(b.last.state, "stStone") then
				disB = disB + 3
			end
		elseif b.state and b.state > 0 and pickGoodAttItem then
	...
	for k, v2 in ipairs(targets) do
		if v2.__cname == "mon" then
			if not self:getTempData(v2, "attacked") and self.settings.ignoreCripple and v2.info.hp and v2.info.hp.max ~= v2.info.hp.cur then
				local preHp = v2.info.hp.cur
				local handler

				handler = scheduler.scheduleGlobal(function()
					if not tolua.isnull(v2.node) then
						if preHp == v2.info.hp.cur then
							v2.info.hp.cur = v2.info.hp.max
						else
							return
						end
					end
				...
				local dis = getDis(player, v2)
				local cls = self:checkMonster(v2)
		...
		elseif player.x == v2.x and player.y == v2.y or self:checkClose(v2) then
			return v2
		end
	end
end
...
function autoRat:isMonster(target)
	return target and (target.__cname == "mon" or target.isCounterAttack)
end
...
function autoRat:isCloseAttack(skipAmulet)
	if self.settings.atkMagic.enable == nil and g_data.player:getMagic(1) then
	...
	if not skipAmulet and self.settings.atkMagic.enable and self.settings.atkMagic.magicId == 48 then
		if def.role.mainsetting and not def.role.mainsetting.noAmulet and not common.checkAmulet(true) then
	...
	if self.settings.atkMagic.magicId == 13 and not self.settings.closeAttack then
		return not self.settings.atkMagic.enable or not g_data.player:getMagic(self.settings.atkMagic.magicId)
	else
...
function autoRat:enable()
	if def.role.mainsetting and def.role.mainsetting.disableAutoRat then
		common.addMsg("本地图禁止挂机。", 255, 252, true)

		return
	end

	if func and func.fixRun and func.fixRun(false) then
		main_scene.ui:tip("行动迟缓状态下，无法挂机！")

		return
	end

	if ccy and ccy.isAutoXiama and ccy.isAutoXiama() then
		return
	end
	...
	main_scene.ui:fadeLabel("开启自动打怪.")

	if not def.closeAutoPick then
		self:switchAutoPickup(true)
	end
...
function autoRat:stop()
	if self.enableRat then
		...
		main_scene.ui:fadeLabel("自动打怪已关闭.")
		self:clearAllAct()

		if not def.closeAutoPick then
			self:switchAutoPickup(false)
		end
	end
end

function autoRat:switchAutoPickup(noAutoPickup)
	g_data.setting.autoRat.noAutoPickup = noAutoPickup

	main_scene.ui.console.btnCallbacks:handle("base", "shiqu")
end
...
function autoRat:clearAllAct()
	...
	self.controller.autoFindPath:multiMapPathStop()
	...
	self.controller.lock:setAttackTarget(nil)
end

function autoRat:getPets(petid)
	local pets = def.role.getPets(petid) or {}
	local pets2 = {}

	for k, v in pairs(main_scene.ground.map.mons) do
		if not v.die then
			for _, item in pairs(pets) do
				if v.info:getRealName() == item then
					v.isPet = true
...
function autoRat:checkPets()
	...
	if not def.role.hasSlaves() then
...
function autoRat:checkRoar()
	...
	if setting.enable and g_data.player:getMagic(43) then
		...
		if self:getRegionRoleCnt(player, 2, "mon") >= setting.cnt then
...
function autoRat:checkCureSelf()
	...
	if setting.enable then
		...
		if player.info.hp.cur / player.info.hp.max * 100 <= setting.percent then
	...
	if setting.enable then
		...
		for _, petid in ipairs({
			...
			for k, v in ipairs(pets) do
				if v.info and v.info.hp.cur and self:getTempData(v, "preHP") ~= v.info.hp.cur and v.info.hp.cur / v.info.hp.max * 100 <= setting.percent then
...
function autoRat:checkPoison()
	if not g_data.client:checkLastTime("magic6", 1.5) then
		return
	end
	...
	if self.settings.autoRangePoison and self.isMonster(self, tar) and not tar.die and getDis(player, tar) <= autoRat.rangeAtkDis and g_data.player:getMagic(183) and not def.role.stateHas(tar.state, "stPoisonRed") and not def.role.stateHas(tar.state, "stPoisonGreen") and tar.poisonTimes > 0 then
		return (self.useMagic(self, tar, 183))
	end

	if self.settings.autoPoison and self.isMonster(self, tar) and not tar.die and getDis(player, tar) <= autoRat.rangeAtkDis and g_data.player:getMagic(6) then
		if def.role.mainsetting and def.role.mainsetting.noAmulet then
			if (not def.role.stateHas(tar.state, "stPoisonRed") or not def.role.stateHas(tar.state, "stPoisonGreen")) and tar.poisonTimes > 0 then
				return (self.useMagic(self, tar, 6))
			end
		else
			local redPoison, greenPoison, equiped = self.console.controller:getPoisonItems(tar)

			if (redPoison and not def.role.stateHas(tar.state, "stPoisonRed") or greenPoison and not def.role.stateHas(tar.state, "stPoisonGreen")) and tar.poisonTimes > 0 then
				return (self.useMagic(self, tar, 6))
			end
		end
	end
...
function explorer:getNext(curPos)
	...
	if not pos or not self:_tryTarget(curPos, pos) then
		...
		for tryCnt = 1, 15 do
			...
			local pos2 = cc.p(startPos.x + x, startPos.y + y)
			local ok = true

			for k, v in ipairs(self.points) do
				if dis > getDis(v, pos2) then
					local index = math.random(#self.points)
			...
			if ok and self:_tryTarget(curPos, pos2) then
				self:addNewPos(pos2)
...
function explorer:_generatePatrolPath()
	...
	;(function()
		...
		for index, _ in ipairs(self.points) do
			...
			for _2, pos in ipairs(self.points) do
...
function autoRat:checkAttackRange()
	...
	local dis = self:isCloseAttack() and 2 or self.atkRangeDis
	...
	if atkMagic and atkMagic.enable and g_data.player:getMagic(atkMagic.magicId) then
		if checkExist(atkMagic.magicId, 1, 5, 13) then
			if not main_scene.ground.map:checkFlyTo(player, self.target) then
		...
		if dis > 3 and self.target and def.role.stateHas(self.target.last.state, "stStone") then
			dis = 3
		end
	end
...
function autoRat:closeToTarget()
	...
	if def.openAutoSpaceLongRat and main_scene.ui.console.controller:autoSpaceLongHitTest() then
		return
	end
	...
	if self:isMonster(self.target) then
		...
		dis = not g_data.player.hitEnables.long and self.atkRangeDis or 3
	end
	...
	if dis then
		if dis > math.abs(player.x - tar.x) and dis > math.abs(player.y - tar.y) then
			return true
		end
...
function autoRat:attack()
	...
	self.controller.lock:setAttackTarget(tar)
...
function autoRat:checkHit()
	...
	if target.info.hp.cur == self.preTargetHP then
		if main_scene.ground.player:isLocked() then
...
function autoRat:useItem(itemName)
	...
	if item then
		...
		if g_data.bag:delItem(item:get("makeIndex")) and self.panels.bag then
			self.panels.bag:delItem(item:get("makeIndex"))
		end
	end
end
...
function autoRat:checkAvoid()
	...
	if self.settings.closeAttack and self.settings.atkMagic.magicId == 13 then
		return
	end

	if self.settings.magicAvoid then
		return
	end
	...
	if self.settings.atkMagic.enable then
		...
		for k2 = 0, 7 do
			wDirs[k2] = 1 / (nearObjsDir[k2] or 0.01)
		end

		local goodDirs = {}

		for k3 = 0, 7 do
			goodDirs[k3] = (wDirs[(k3 + 7) % 8] or 0) / 3 + (wDirs[(k3 + 1) % 8] or 0) / 3 + wDirs[k3]
		end

		local max = 0

		for k4, v2 in pairs(goodDirs) do
			local config = def.role.dir["_" .. k4]
			...
			if max < v2 then
				max = v2
				dir = k4
			elseif max == v2 and math.random() < 0.5 then
				dir = k4
			end
		end

		local config2 = def.role.dir["_" .. dir]

		if not self:isCanWalk(player.x + config2[1], player.y + config2[2]) then
		...
		if not self:isCanWalk(player.x + config2[1] * 2, player.y + config2[2] * 2) then
			step = 1
		end
...
function autoRat:checkAutoSpaceMove()
	...
	if setting.enable then
		...
		if os.time() - self.lastExpUpdateTime < setting.space * 60 then
			return false
		end
...
function autoRat:autoSkill()
	if not self.target or self.target.die then
		return
	end
	...
	for _, csSkill in pairs(def.csSkills) do
		if (csSkill.job == 3 or csSkill.job == g_data.player.job) and csSkill.autoSkill and g_data.setting.job[csSkill.key] then
			local magic2 = g_data.player:getMagic(csSkill.magicId)

			if magic2 then
				local magicConfigByUid = def.magic.getMagicConfigByUid(csSkill.magicId, player)

				if magicConfigByUid then
					local value = def.ccy.calcCDTime(magic2.delayTime, magicConfigByUid, csSkill.cdtime)
					local text = "magic" .. tostring(csSkill.magicId)

					if csSkill.type == "lockImmediateSill" and g_data.client:checkLastTime(text, value) and getDis(player, self.target) <= self.rangeAtkDis then
						if csSkill.isDBSkill then
							self.console:call("lock", "setSkillTarget", self.target)
							self.console:call("lock", "useSkill", magic2, magicConfigByUid)

							if self.controller:useMagic() then
								g_data.player:setHitEnable(csSkill.key, false)

								break
							end
						else
							self.controller:callSkill(csSkill.key)
...
function autoRat:executeTest(dt)
	...
	if self.controller:checkZhanjiashu() then
		-- block empty
	elseif self.controller:checkYoulingDun() then
		-- block empty
	end

	if g_data.player.job ~= 2 and g_data.player:getMagic(6) then
		self:checkPoison()
	end

	if g_data.player.job ~= 0 or not self:checkRoar() then
		if g_data.player.job == 2 or def.ccy.useSC() then
			if not self:checkPets() and not self:checkCureSelf() and not self:checkCurePet() and (not self:checkPoison() or false) then
