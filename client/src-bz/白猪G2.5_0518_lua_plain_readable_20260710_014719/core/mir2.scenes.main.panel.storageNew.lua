...
function storage:ctor(merchant, count, gridCnt, buf, bufLen)
	...
	self:anchor(1, 1):pos(display.cx - 5, display.height - 32):size(cc.size(value_2:getContentSize().width, value_2:getContentSize().height)):scale(1)
	an.newBtn(res.gettex2("pic/panels/bag/close10.png"), function()
		...
		size = cc.size(64, 64)
	}):anchor(0, 1):pos(self:getw() - 5, self:geth()):addto(self, 2)
	an.newBtn(res.gettex2("pic/panels/bag/bz_2.png"), function()
		...
		select = {
			res.gettex2("pic/panels/bag/bz_1.png")
		}
	}):addTo(self, 2):pos(60, 97):anchor(0, 0.5)
	an.newBtn(res.gettex2("pic/panels/bag/bz_2.png"), function()
		sound.playSound("103")
...
function storage:reload()
	...
	for i = 1, self.gridMax do
		...
		if self:gridIsOpen(idx) then
			display.newScale9Sprite(res.getframe2("pic/panels/storage/icon_lock_bg.png")):size(57, 47):addto(self, 2):pos(x, y)
			...
			local item2 = display.newScale9Sprite(res.getframe2("pic/panels/storage/icon_lock_bg.png")):size(57, 47):addto(self, 2):pos(x, y)
...
function storage:idx2pos(idx)
	...
	return 48 + h * 56, 375 - v * 47
end

function storage:pos2idx(x, y)
	local h = (x - 48) / 56 + 0.5
	local v = (375 - y) / 47 + 0.5
