...
function bag:resetPanelPosition(type)
	...
	elseif type == "storage" then
		self:anchor(0, 1):pos(display.cx + 15, display.height - 26)
	end
...
function bag:gotoPage(pageIdx)
	for index = 1, 4 do
		if items[index] then
			...
			items[index]:setPositionX(self:getw() - 30)
		end
	end
	...
	if items[pageIdx] then
		...
		items[pageIdx]:setPositionX(self:getw() - 28)
	end
...
function bag:ctor()
	...
	an.newBtn(res.gettex2("pic/panels/bag/close10.png"), function()
		...
		size = cc.size(64, 64)
	}):anchor(1, 1):pos(self:getw() - 5, self:geth() - 20):addto(self):setName("bag_close")
	...
	if def.enableHuishou then
		...
		}):pos(275, 82):add2(self):scale(0.8)
	end
	...
	an.newBtn(res.gettex2("pic/panels/bag/zlbb.png"), function()
		...
		pressImage = res.gettex2("pic/panels/bag/zlbb2.png")
	}):pos(348, 82):add2(self):scale(0.8)
	...
	if def.enableRemoteWarehouse then
		...
		}):pos(430, 80):add2(self)
	end

	if g_data.bag.max > number then
		local w = self:getw() - 30
		local y = 353
		local number2 = 55
		...
		}):pos(w, y):add2(self):anchor(0, 1):scale(0.8)
		...
		}):pos(w, y - number2):add2(self):anchor(0, 1):scale(0.8)
		...
		}):pos(w, y - number2 * 2):add2(self):anchor(0, 1):scale(0.8)
		...
		}):pos(w, y - number2 * 3):add2(self):anchor(0, 1):scale(0.8)

		items[self.pageIdx]:select()
		items[self.pageIdx]:setLocalZOrder(999)
		items[self.pageIdx]:setPositionX(self:getw() - 28)
	end
	...
	}):addto(self):pos(25, 80)

	self.gold = an.newLabel(change2GoldStyle(value), 14, 0, {
		color = cc.c3b(255, 255, 255)
	}):pos(60, 73):addto(self)

	if def.openBagYB then
		res.get2("pic/panels/bag/bz_diy_yb.png"):addTo(self):pos(50, 37):anchor(0, 0):scale(0.75)

		self.yb = an.newLabel(change2GoldStyle(value2), 14, 0, {
			color = cc.c3b(255, 255, 255)
		}):pos(100, 39):addto(self)
	end

	if def.openBagLF then
		res.get2("pic/panels/bag/bz_diy_lf.png"):addTo(self):pos(50, 16):anchor(0, 0):scale(0.75)

		self.lingfu = an.newLabel(change2GoldStyle(value3), 14, 0, {
			color = cc.c3b(255, 255, 255)
		}):pos(100, 18):addto(self)
	end

	if g_data.hero and g_data.hero.roleid > 0 then
		display.newScale9Sprite(res.getframe2("pic/scale/scale22.png")):size(50, 48):addto(self):pos(self:getw() - 59, 36)
...
function bag:idx2pos(idx)
	...
	return 60 + h * 48.5, 370 - v * 48.5
end

function bag:pos2idx(x, y)
	local h = (x - 48) / 50.5 + 0.5
	local v = (370 - y) / 50.5 + 0.5
	local number2 = -1
