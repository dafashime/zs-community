	...
	local function appRun()
		...
		if type(import) == "function" then
			...
			import = function(...)
				local name = select(1, ...)
				local resolved = nil
				-- fix relative import resolution for zip-loaded chunks:
				-- quick's import() falls back to debug.getlocal(3,1) which does
				-- NOT see the module name for chunks loaded from ZIP. Resolve
				-- the relative path ourselves from the caller chunk's name.
				if type(name) == "string" and string.byte(name, 1) == 46 and select("#", ...) < 2 then
					local info = debug.getinfo(2, "S")
					local src = info and info.source or ""
					local m = src:match('%["([^"]+)%"%]') or src:match('([^%\\/]+)%.lua$') or src:match('([^%\\/]+)$')
					local cur = m and m:gsub('%.lua$', '') or nil
					if cur then
						local parts = {}
						for p in string.gmatch(cur, "[^%.]+") do parts[#parts + 1] = p end
						local full = name
						local off = 1
						while true do
							if string.byte(name, off) ~= 46 then
								full = string.sub(name, off)
								if #parts > 0 then full = table.concat(parts, ".") .. "." .. full end
								break
							end
							off = off + 1
							table.remove(parts, #parts)
						end
						slog("HK", "import", "REL", tostring(name), "->", tostring(full), "from", tostring(cur))
						resolved = full
					end
				end
				local ok2, ret
				if resolved then
					ok2, ret = pcall(oi, resolved)
				else
					ok2, ret = pcall(oi, ...)
				end
