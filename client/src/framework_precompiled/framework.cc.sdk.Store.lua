...
function Store.setReceiptVerifyMode(mode, isSandbox)
	...
	if type(mode) ~= "number" or mode ~= cc.CCStoreReceiptVerifyModeNone and mode ~= cc.CCStoreReceiptVerifyModeDevice and mode ~= cc.CCStoreReceiptVerifyModeServer then
		printError("Store.setReceiptVerifyMode() - invalid mode")

		return false
	end

	if type(isSandbox) ~= "boolean" then
		isSandbox = true
	end
