...
function UIBoxLayout:apply(container)
	if table.nums(self.widgets_) == 0 then
		return
	end
	...
	local totalWeightH = 0
	local totalWeightV = 0
	local fixedWidth = 0
	local fixedHeight = 0
	local maxWidth = 0
	local maxHeight = 0
	...
	for widget, v in pairs(self.widgets_) do
		...
		if maxWidth < widgetSizeWidth then
			maxWidth = widgetSizeWidth
		end

		if maxHeight < widgetSizeHeight then
	...
	local x, y, negativeX, negativeY = nil
	...
	for index, item in ipairs(widgets) do
		...
		local actualWidth, actualHeight = nil
