...
UISlider.BARFG_ZORDER = 1
UISlider.BUTTON_ZORDER = 2

function UISlider:ctor(direction, images, options)
	self.fsm_ = {}

	cc(self.fsm_):addComponent("components.behavior.StateMachine"):exportMethods()
	...
	self.buttonPositionRange_ = {
		min = 0,
		max = 0
	}
	self.buttonPositionOffset_ = {
		x = 0,
		y = 0
	}
...
function UISlider:setSliderEnabled(enabled)
	...
	if enabled and self.fsm_:canDoEvent("enable") then
		...
		self:dispatchEvent({
			name = UISlider.STATE_CHANGED_EVENT,
...
function UISlider:setSliderValue(value)
	assert(self.min_ <= value and value <= self.max_, "UISlider:setSliderValue() - invalid value")
	...
	if self.value_ ~= value then
		...
		self:updateButtonPosition_()
		self:dispatchEvent({
			name = UISlider.VALUE_CHANGED_EVENT,
			value = self.value_
		})
	end
...
function UISlider:onTouch_(event, x, y)
	...
	if self.isHorizontal_ then
		...
		elseif self.buttonPositionRange_.max < x then
		...
		elseif self.buttonPositionRange_.max < y then
...
function UISlider:checkTouchInButton_(x, y)
	if not self.buttonSprite_ then
		return false
	end
...
function UISlider:updateButtonPosition_()
	if not self.barSprite_ or not self.buttonSprite_ then
		return
	end
...
function UISlider:updateImage_()
	...
	if barfgImage and not self.barfgSprite_ then
		if self.scale9_ then
