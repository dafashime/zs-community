...
function scene:onAssetError(code, msg, curle, curlm)
	...
	elseif code == assetUpter.EventCode.ERROR_UPDATING or eventCode == assetUpter.EventCode.UPDATE_FAILED then
		...
		msgbox.new(string.format("网络异常 error :%s. %d,%d", msg or "update faild", curle, curlm), function (b)
			os.exit(0)
		end)
	end
end
...
function scene:checkData_android(func)
	...
	local function copy()
		if #tasks > 0 then
			...
			self:setTitle(string.format("首次进入游戏正在初始化(%.1f％)..", p * 100))
			self:setProgress(p * 100)
