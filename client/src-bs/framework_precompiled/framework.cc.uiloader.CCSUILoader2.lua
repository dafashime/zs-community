...
function CCSUILoader:getCheckBoxImages(options)
	...
	images.on = {
		images.off,
		self:transResName(options.NodeNormalFileData)
	}
...
function CCSUILoader:transResName(fileData)
	...
	if fileData.Type == "PlistSubImage" then
		return "#" .. name
	else
...
function CCSUILoader:createNode(options)
	...
	node:setAnchorPoint(cc.p(options.AnchorPoint.ScaleX or 0, options.AnchorPoint.ScaleY or 0))
...
function CCSUILoader:createImage(options)
	...
	if params.scale9 then
		params.capInsets = cc.rect(options.Scale9OriginX, options.Scale9OriginY, options.Scale9Width, options.Scale9Height)
	end
...
function CCSUILoader:createButton(options)
	...
	node:align(self:getAnchorType(options.AnchorPoint.ScaleX or 0, options.AnchorPoint.ScaleY or 0), options.Position.X or 0, options.Position.Y or 0)
...
function CCSUILoader:createLoadingBar(options)
	local params = {
		image = self:transResName(options.ImageFileData),
		scale9 = options.Scale9Enable,
		capInsets = cc.rect(options.Scale9OriginX, options.Scale9OriginY, options.Scale9Width, options.Scale9Height)
	}

	if options.ProgressType == "Right_To_Left" then
...
function CCSUILoader:createLabel(options)
	...
	if options.HorizontalAlignmentType == "HT_Center" then
		hAlignment = cc.TEXT_ALIGNMENT_CENTER
	elseif options.HorizontalAlignmentType == "HT_Right" then
		hAlignment = cc.TEXT_ALIGNMENT_RIGHT
	else
		hAlignment = cc.TEXT_ALIGNMENT_LEFT
	end

	local vAlignment = nil

	if options.VerticalAlignmentType == "VT_Center" then
		vAlignment = cc.VERTICAL_TEXT_ALIGNMENT_CENTER
	elseif options.VerticalAlignmentType == "VT_Bottom" then
...
function CCSUILoader:createEditBox(options)
	...
	editBox:setAnchorPoint(cc.p(options.AnchorPoint.ScaleX or 0, options.AnchorPoint.ScaleY or 0))
...
function CCSUILoader:createPanel(options)
	...
	if options.ComboBoxIndex == 1 then
		...
		clrLayer:setColor(cc.c3b(options.SingleColor.R or 255, options.SingleColor.G or 255, options.SingleColor.B or 255))
	elseif options.ComboBoxIndex == 2 then
		...
		clrLayer:setTouchEnabled(false)
		clrLayer:setStartColor(cc.c3b(options.FirstColor.R or 255, options.FirstColor.G or 255, options.FirstColor.B or 255))
		clrLayer:setEndColor(cc.c3b(options.EndColor.R or 255, options.EndColor.G or 255, options.EndColor.B or 255))
	...
	if options.Scale9Enable then
		if options.FileData and options.FileData.Path then
			local capInsets = cc.rect(options.Scale9OriginX, options.Scale9OriginY, options.Scale9Width, options.Scale9Height)
			local scale9sp = ccui.Scale9Sprite or cc.Scale9Sprite

			if options.FileData.Type == "Normal" then
				bgLayer = scale9sp:create(capInsets, options.FileData.Path)
			else
				bgLayer = scale9sp:createWithSpriteFrameName(options.FileData.Path, capInsets)
			end

			bgLayer:setContentSize(cc.size(options.Size.X, options.Size.Y))
		end
	elseif options.FileData and options.FileData.Path then
		bgLayer = display.newSprite(self:transResName(options.FileData))
	end
...
function CCSUILoader:createScrollView(options)
	...
	if options.ScrollDirectionType == "Vertical" then
		dir = 1
	elseif options.ScrollDirectionType == "Horizontal" then
		dir = 2
	else
		dir = 0
	end
...
function CCSUILoader:createListView(options)
	...
	if options.DirectionType == "Vertical" then
		dir = 1
	elseif options.DirectionType == "Horizontal" then
		dir = 2
	else
		dir = 0
	end
